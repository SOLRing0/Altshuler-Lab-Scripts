### COMPARE THE EFFECTS OF CYCLE NORMALIZED STIMULUS DURATION ###
...
### 50% stimulus duration at 8.6 Hz ###
 ...
 shapiro.test(residuals(phases2)) # good
 summary(phases2)
 anova(phases2)
...
# Likelihood ratio test
out.stim.phases50<-anova(phases2.null, phases2)

##### calculate the summary statistics from the fit for each individual and then run an anova--lm() on summary stats~treatment

#### calculate the summary statistics for 8.6 Hz, 50% stimulus duration ####
head(out_50pc)
rm(data50)
data50<- subset(data, c(Freq_Hz==8.6 & Stim_dur_pc_cyc==50))
data50$fitted<-fitted(out_50pc)
...
data50$Bird_code[1:22]
data50$Bird_code[23:44]
# data50$fitted[1:22] is COLLI-039 aka Bird 5
# data50$fitted[23:44] is COLLI-041 aka Bird 6

summaryPower50 <- data.frame( stim = 50, Subject = c("Bird 5", "Bird 6"),
	mean = c(mean(data50$fitted[1:22]), mean(data50$fitted[23:44])),
	min = c(range(data50$fitted[1:22])[1], range(data50$fitted[23:44])[1]),
	max = c(range(data50$fitted[1:22])[2], range(data50$fitted[23:44])[2]),
	range = c(diff(range(data50$fitted[1:22])),diff(range(data50$fitted[23:44]))),
	Act = c((length(data50$Phase_pc[which(data50$fitted[1:22] >= 0)])/(length(unique(data50$Phase_pc))))*100,
		 (length(data50$Phase_pc[which(data50$fitted[23:44] >= 0)])/(length(unique(data50$Phase_pc))))*100) )
...
avg_min = c(mean(summaryPower50$min), mean(summaryPower69$min)),
SE_min = c(std.error(summaryPower50$min), std.error(summaryPower69$min)),
avg_max = c(mean(summaryPower50$max), mean(summaryPower69$max)),
SE_max = c(std.error(summaryPower50$max), std.error(summaryPower69$max)),
avg_mean = c(mean(summaryPower50$mean), mean(summaryPower69$mean)),
SE_mean = c(std.error(summaryPower50$mean), std.error(summaryPower69$mean)),
avg_act = c(mean(summaryPower50$Act), mean(summaryPower69$Act)),
SE_act = c(std.error(summaryPower50$Act), std.error(summaryPower69$Act)) )

## create a single dataframe containing all of the summary stats of interest
Pow.summ.stats <-rbind(summaryPower50, summaryPower69)
...
stim.summstatstable$p_values.phase_effects<-c(out.stim.phases50$'p-value'[2],out.stim.phases69$'p-value'[2],NA)

# write csv of summary stats and p-values
write.csv(stim.summstatstable,"summary stats and p-values--stimulus durations.csv",row.names=FALSE)
...
# Add legend
legend(-48,-400, legend = c("Bird 5","Bird 6"), 
...
plot( mean~stim, data=Pow.summ.stats,
      ...
      pch=c(1,2,15:18)[as.factor(Pow.summ.stats$Subject)], bty="n", tcl=0.5,
...
points( min~stim, data=Pow.summ.stats,
	cex=1,
	col=c("red","purple")[as.factor(Pow.summ.stats$stim)],
	pch=c(1,2,15:18)[as.factor(Pow.summ.stats$Subject)])
abline(h=0, lty="dashed")
axis(1, at=c(40,50,69,80), labels=FALSE, tcl=0.5)
